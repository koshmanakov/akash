...
test-e2e-integration: export KIND_K8S_IP=$(KIND_API_IP)
test-e2e-integration: export KIND_APP_PORT=$(KIND_HTTP_PORT)
...
test-e2e-integration: $(BINS)
	...
	cp akashctl akashd ./_build
	go test -mod=readonly -p 4 -tags "e2e integration $(BUILD_MAINNET)" -v ./integration/... -run TestE2EApp

test-query-app: $(BINS)
	KIND_APP_PORT="$(KIND_HTTP_PORT)" KIND_K8S_IP="$(KIND_API_IP)" go test -mod=readonly -p 4 -tags "e2e integration $(BUILD_MAINNET)" -v ./integration/... -run TestQueryApp
